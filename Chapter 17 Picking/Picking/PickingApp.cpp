...
void PickingApp::Pick(int sx, int sy)
{
	...
	for (auto ri : mRitemLayer[(int)RenderLayer::Opaque])
	{
		...
		// 因此,我们就应进一步执行射线与三角形的相交测试

		float tmin = 0.0f;
		if (ri->Bounds.Intersects(rayOrigin, rayDir, tmin)) // intersect: 相交
		{
			// 对于此demo,我们知道如何强制转换顶点与索引的数据格式
			// 对于不同格式混合在一起的情况,需要用元数据(metadata)来执行强制类型转换
			...
			for (UINT i = 0; i < triCount; ++i)
			{
				...
				// 遍历网格上的所有三角形,以找到距摄像机最近的三角形
				float t = 0.0f;
				if (TriangleTests::Intersects(rayOrigin, rayDir, v0, v1, v2, t))
				{
					if (t < tmin)
					{
						// 这是目前距离相机最近的一个被拾取的三角形
						tmin = t;
