...
private:
	...
	XMFLOAT4X4 mShadowTransform = MathHelper::Identity4x4(); // 将物体从世界空间变换到纹理空间
...
void ShadowMapApp::BuildRootSignature()
{
	...
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0, 0); // TextureCube gCubeMap : register(t0)

	CD3DX12_DESCRIPTOR_RANGE texTable1;
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 2, 0); // Texture2D gTextureMaps[10] : register(t2)
...
void ShadowMapApp::BuildDescriptorHeaps()
{
	...
	mShadowMap->BuildDescriptors(
		...
		CD3DX12_CPU_DESCRIPTOR_HANDLE(dsvCpuStart, 1, mDsvDescriptorSize)); // 深度缓存偏移1
}
...
void ShadowMapApp::BuildSkullGeometry()
{
	...
	for (UINT i = 0; i < vcount; ++i)
	{
		...
		if (fabsf(XMVectorGetX(XMVector3Dot(N, up))) < 1.0f - 0.001f) // cos 越小,夹角越大
		{
...
void ShadowMapApp::BuildPSOs()
{
	...
	opaquePsoDesc.SampleDesc.Count = m4xMsaaState ? 4 : 1; // false
	...
	smapPsoDesc.RasterizerState.DepthBias = 100000; // 深度偏移,用于防止锯齿,有特定的计算方式
...
void ShadowMapApp::DrawSceneToShadowMap()
{
	...
	// 由于仅向深度缓冲区绘制数据,因此将渲染目标设为空,这样一来禁止颜色数据向渲染目标的写操作
	// 此时要把PSO中的渲染目标数量指定为0
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> ShadowMapApp::GetStaticSamplers()
{
	...
	const CD3DX12_STATIC_SAMPLER_DESC shadow(
		6, // shaderRegister
		D3D12_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT, // filter 阴影贴图的过滤器
		D3D12_TEXTURE_ADDRESS_MODE_BORDER,  // addressU,U轴寻址模式
		D3D12_TEXTURE_ADDRESS_MODE_BORDER,  // addressV
		D3D12_TEXTURE_ADDRESS_MODE_BORDER,  // addressW
		0.0f,                               // mipLODBias mipmap层级偏移
		16,                                 // maxAnisotropy
